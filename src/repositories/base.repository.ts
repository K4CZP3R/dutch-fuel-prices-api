...
export class BaseRepository<T extends { _id?: string }> implements IBaseRepository<T> {
    ...
    async removeAll(): Promise<number> {
        return await this.Model.remove({}).count()
    }

    async addObject(item: T): Promise<T> {
    ...
    async getWithFilters(filters?: { [key: string]: any }, orderBy?: { [key: string]: number }, limit?: number): Promise<T[]> {
        return limit ? (await this.Model.find(filters).sort(orderBy).limit(limit)) as T[] : (await this.Model.find(filters).sort(orderBy)) as T[];
    }
